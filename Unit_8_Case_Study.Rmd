---
title: "Advanced analytics on Budweiser data"
...
Beer_Breweries_raw$Beer_Category <- ifelse(str_detect(Beer_Breweries_raw$Beer_Style, "IPA"), 'IPA', ifelse(str_detect(Beer_Breweries_raw$Beer_Name, "Ale"),'Other Ale','Other'))
...
message("By running KNN for different samples and different values of K, we can see the knn model accuracy is highest at k=3. So performing internal validation at KNN at k=3")
#Perform internal validation with knn with knn=3

knn3 <- knn.cv(Beer_Breweries_8_knn_data[, c(3,4)],Beer_Breweries_8_knn_data$Beer_Category, k=3 )

t_knn <- table(knn3,Beer_Breweries_8_knn_data$Beer_Category )
...
message("We can see that around 23 beers were misclassified as 'IPA' and 51 were misclassified as 'Other Ale' ")
...
nrow(Mismatched_data)
message("In total there are 82 beers, out of 2410, which, according to the KNN model, seem to have been incorrectly categoried as IPA or Other Ales by Budweiser")
...
82 beers are candidates for re-classification for Beer Style based on their Alcohol content and bitterness levels. 
